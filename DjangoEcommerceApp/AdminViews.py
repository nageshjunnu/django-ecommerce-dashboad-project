...
from DjangoEcommerceApp.models import Categories, SubCategories, MerchantUser, CustomUser,Products,ProductAbout,ProductDetails,ProductMedia,ProductTransaction,ProductTags
...
class MerchantUserCreate(SuccessMessageMixin, CreateView):
    ...
    def form_valid(self,form):
        is_private = self.request.POST.get('is_private', False)
...
class ProductView(View):
    def get(self, request, *args, **kwargs):
        ...
        for category in categories:
            sub_categories = SubCategories.objects.filter(is_active=1, category_id=category.id)
            categories_list.append({"category":category, "sub_category":sub_categories})
